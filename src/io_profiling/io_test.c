...
int main(int argc, char *argv[]) {
  ...
  You 're absolutely right, and I apologize for overlooking that detail. Let' s
      modify the program to handle this correctly
          .We 'll split the input dataset into the directory and field name, and construct the full path appropriately. Here' s
              the updated version
      : cCopy #include<libpressio.h>
#include <fcntl.h>
#include <libgen.h>
#include <libpressio_ext/io/posix.h>
#include <math.h>
...
#define BUFFER_SIZE 8192
  ...
  int main(int argc, char *argv[]) {
    ...
    while (iteration < MAX_ITERATIONS && !confidence_interval_reached) {
      ...
      start_time = get_time();
      struct pressio_data *decompressed_data =
          pressio_data_new_clone(input_data);
      ...
      fprintf(csv_file, "%s,%s,%s,%f,%d,%f,%f,%f,%f,%f,%f\n", compressor_id,
              ...
              write_compressed_times[iteration],
              read_compressed_times[iteration], decompression_times[iteration],
